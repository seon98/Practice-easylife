...
def test_signup_login_me_and_conflict(client: TestClient) -> None:
    ...
    assert client.post("/api/v1/auth/login", json={**payload, "password": "wrong"}).status_code == 401
...
def test_signup_validation(client: TestClient) -> None:
    assert client.post("/api/v1/auth/signup", json={"email": "invalid", "password": "short"}).status_code == 422
