...
async def merge_anonymous_favorites(
    session: AsyncSession, client_id: UUID, user: UserModel
) -> None:
    existing = set(
        await session.scalars(
            select(FavoriteModel.service_id).where(FavoriteModel.user_id == user.id)
        )
    )
    await session.execute(
        ...
        .where(
            ...
            FavoriteModel.service_id.not_in(existing),
        )
